	...
	for (int x = 9; x <= 35; x = x + 8) {       //输出数字,x,y为输出坐标
		...
		for (int y = 5; y <= 17; y = y + 4) {
			...
			if (GameSquare[ax][ay]) {
				...
				OutPoint.write(x - 3, y - 1, "┌");//左上角
				OutPoint.write(x - 2, y - 1, "────");//上
				OutPoint.write(x + 2, y - 1, "┐");//右上角
				OutPoint.write(x + 2, y, "│");//右
				OutPoint.write(x + 2, y + 1, "┘");//右下角
				OutPoint.write(x - 2, y + 1, "────");//下
				OutPoint.write(x - 3, y + 1, "└");//左下角
				OutPoint.write(x - 3, y, "│");//左
